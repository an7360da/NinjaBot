...
//					System.out.println("NinjaBot broadcasts: targetEnemy;" + Robot.getEnemyLeader());
	...
	public void shoot() {
		...
		if (getGunTurnRemaining() == 0 && Robot.getEnergy() > 5 && Robot.hasTarget()) {
			...
			setFire(Math.min(Math.min(Robot.getEnergy() / 6d, 1300d / Robot.getDistanceToTarget()),
					Robot.getTarget().getEnergy() / 3d));
			setTurnGunRightRadians(Utils.normalRelativeAngle(
					Calculations.calcAngle(Robot.getTarget().getPosition(), Robot.getPos()) - getGunHeadingRadians()));
		}
	}
...
//- scan event ------------------------------------------------------------------------------------------------------------------------------
	...
	public void onScannedRobot(ScannedRobotEvent e) {
		...
		if (isTeammate(e.getName())) {
			...
			try {
				System.out.println("friendPos: " + scannedRobot.getName() + ";" + scannedRobot.getPosition().x + ";"
						+ scannedRobot.getPosition().y);
				broadcastMessage("friendPos;" + scannedRobot.getName() + ";" + scannedRobot.getPosition().x + ";"
						+ scannedRobot.getPosition().y);
			} catch (IOException e1) {
				e1.printStackTrace();
			}
			...
			try {
				System.out.println(
						"targetPos: " + scannedRobot.getPosition().x + ";" + scannedRobot.getPosition().getY());
			...
			for (int i = 0; i < Environment.friends.size(); i++) {
				...
				if (targetValue > 7) {
					try {
						System.out.println(
								"Message to: " + teammate.getName() + " targetEnemy:" + scannedRobot.getName());
